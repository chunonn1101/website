...
export function StaticPage({ type }) {
  ...
  return <main className="text-page page-shell"><Breadcrumb items={[{ label: title }]} /><div className="text-page-grid"><div><p className="eyebrow">CUSTOMER CARE</p><h1>{title}</h1></div><article><h2>{heading}</h2><p>{copy}</p>{type === "contact" && <div className="address-card"><p className="eyebrow gold">UAE REGISTERED COMPANY</p><h3>Modex Fashion Trading FZE</h3><p>Business Centre,<br />Sharjah Publishing City Free Zone,<br />Sharjah, United Arab Emirates</p><p className="license-number">Business License No. 4420464.01</p></div>}{type === "faq" && <><h3>How are pieces authenticated?</h3><p>Our specialists examine construction, materials, hardware, codes and available provenance before final quality control.</p><h3>Do you ship internationally?</h3><p>Yes. Available delivery services and estimated duties are shown during checkout.</p><h3>Can I sell to MODEX?</h3><p>Yes. Submit details and photographs through Sell With Us for an initial evaluation.</p></>}</article></div></main>;
}
